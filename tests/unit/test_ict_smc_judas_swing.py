from datetime import datetime, timedelta, timezone

from src.analytics.ict_smc.judas_swing import detect_judas_swing


def _row(index, minutes, open_p, high_p, low_p, close_p, timeframe="5m", is_closed=True):
    return {
        "index": index,
        "timestamp": datetime(2026, 6, 4, tzinfo=timezone.utc) + timedelta(minutes=minutes),
        "symbol": "XAUUSD",
        "timeframe": timeframe,
        "open": open_p,
        "high": high_p,
        "low": low_p,
        "close": close_p,
        "volume": 100 + index,
        "is_closed": is_closed,
    }


def _session_range():
    return {
        "session_name": "asian_session",
        "range_high": 2368.40,
        "range_low": 2356.20,
        "range_midpoint": 2362.30,
        "range_size": 12.20,
        "session_start": "00:00",
        "session_end": "06:00",
        "timezone": "UTC",
        "quality_score": 8.0,
        "session_end_index": 4,
    }


def _accumulation_rows():
    return [
        _row(0, 0, 2360.0, 2363.0, 2358.0, 2361.0),
        _row(1, 60, 2361.0, 2368.4, 2360.8, 2365.7),
        _row(2, 120, 2365.7, 2366.5, 2356.2, 2358.5),
        _row(3, 240, 2358.5, 2364.8, 2357.0, 2362.0),
        _row(4, 360, 2362.0, 2365.0, 2359.2, 2361.8),
    ]


def test_valid_bullish_judas_swing_scores_high_quality() -> None:
    rows = _accumulation_rows() + [
        _row(5, 430, 2358.0, 2358.8, 2352.8, 2357.4),
        _row(6, 435, 2357.4, 2361.8, 2356.9, 2361.2),
        _row(7, 440, 2361.2, 2367.0, 2360.8, 2366.4),
        _row(8, 445, 2366.4, 2374.5, 2366.2, 2373.8),
        _row(9, 450, 2373.8, 2378.0, 2372.5, 2376.4),
    ]

    result = detect_judas_swing(
        rows,
        _session_range(),
        htf_bias="bullish",
        sweep_buffer=0.25,
        close_buffer=0.25,
        mss_events=[{"direction": "bullish", "index": 7, "broken_level": 2361.8}],
        fvg_events=[{"fvg_type": "bullish_fvg", "creation_index": 8, "zone_low": 2361.8, "zone_high": 2366.2}],
        active_session="london_killzone",
    )

    assert result["judas_type"] == "bullish_judas"
    assert result["manipulation_side"] == "below_range"
    assert result["swept_liquidity"] == "sell_side"
    assert result["reclaim_status"] == "reclaimed_back_inside_range"
    assert result["mss_confirmed"] is True
    assert result["entry_zone"]["entry_zone_type"] == "bullish_fvg"
    assert result["target_side"] == "buy_side"
    assert result["targets"]["second_target"]["target_type"] == "session_high"
    assert result["risk_logic"]["entry_allowed_from_judas_sweep_alone"] is False
    assert result["quality_score"] >= 8.0


def test_valid_bearish_judas_swing_scores_strong() -> None:
    rows = _accumulation_rows() + [
        _row(5, 430, 2366.2, 2372.6, 2365.8, 2367.5),
        _row(6, 435, 2367.5, 2368.0, 2362.0, 2362.5),
        _row(7, 440, 2362.5, 2363.0, 2357.1, 2357.5),
        _row(8, 445, 2357.5, 2358.2, 2349.5, 2350.2),
        _row(9, 450, 2350.2, 2352.0, 2346.0, 2347.4),
    ]

    result = detect_judas_swing(
        rows,
        _session_range(),
        htf_bias="bearish",
        sweep_buffer=0.25,
        close_buffer=0.25,
        mss_events=[{"direction": "bearish", "index": 7, "broken_level": 2362.0}],
        fvg_events=[{"fvg_type": "bearish_fvg", "creation_index": 8, "zone_low": 2358.2, "zone_high": 2362.0}],
        active_session="newyork_killzone",
    )

    assert result["judas_type"] == "bearish_judas"
    assert result["manipulation_side"] == "above_range"
    assert result["swept_liquidity"] == "buy_side"
    assert result["reclaim_status"] == "rejected_back_inside_range"
    assert result["mss_confirmed"] is True
    assert result["entry_zone"]["entry_zone_type"] == "bearish_fvg"
    assert result["target_side"] == "sell_side"
    assert result["targets"]["second_target"]["target_type"] == "session_low"
    assert result["quality_score"] >= 8.0


def test_manipulation_without_mss_stays_candidate_and_blocks_entry() -> None:
    rows = _accumulation_rows() + [
        _row(5, 430, 2358.0, 2358.8, 2352.8, 2357.4),
        _row(6, 435, 2357.4, 2359.0, 2356.5, 2358.0),
        _row(7, 440, 2358.0, 2360.0, 2357.2, 2359.0),
    ]

    result = detect_judas_swing(
        rows,
        _session_range(),
        htf_bias="bullish",
        sweep_buffer=0.25,
        close_buffer=0.25,
        mss_events=[],
        fvg_events=[],
    )

    assert result["judas_type"] == "bullish_judas_candidate"
    assert result["mss_confirmed"] is False
    assert result["entry_zone"] is None
    assert result["risk_logic"]["entry_allowed_after_fvg_or_ob_retest_reaction"] is False
    assert result["quality_score"] <= 5.0
    assert "wait_for_MSS_before_treating_as_valid_judas" in result["warnings"]


def test_breakout_acceptance_is_not_bearish_judas() -> None:
    rows = _accumulation_rows() + [
        _row(5, 430, 2367.0, 2373.8, 2366.8, 2372.9),
        _row(6, 435, 2372.9, 2378.4, 2372.0, 2377.0),
    ]

    result = detect_judas_swing(
        rows,
        _session_range(),
        htf_bias="bullish",
        sweep_buffer=0.25,
        close_buffer=0.50,
    )

    assert result["judas_type"] == "invalid_bearish_judas_candidate"
    assert result["classification"] == "bullish_breakout_continuation_not_bearish_judas"
    assert result["reclaim_status"] == "accepted_above_range"
    assert result["quality_score"] <= 3.0
    assert "price_accepted_above_range_not_bearish_judas" in result["warnings"]


def test_weak_tiny_wick_false_positive_is_low_quality() -> None:
    rows = _accumulation_rows() + [
        _row(5, 430, 2356.4, 2358.0, 2355.8, 2355.95),
        _row(6, 435, 2355.95, 2357.2, 2355.9, 2356.1),
    ]

    result = detect_judas_swing(
        rows,
        _session_range(),
        htf_bias="bearish",
        sweep_buffer=0.20,
        close_buffer=0.50,
        mss_events=[],
        fvg_events=[],
    )

    assert result["judas_type"] == "weak_bullish_judas_candidate"
    assert result["classification"] == "unclear_downside_raid"
    assert result["reclaim_status"] == "unclear"
    assert result["mss_confirmed"] is False
    assert result["entry_zone"] is None
    assert result["quality_score"] <= 4.0
