...
### R-05: Final quote-time execution validation and inactive-feed rejection
...
**Before:** A candidate could pass risk approval at its setup price while the MT5 gateway later substituted a newer executable quote without rechecking currency risk, spread or trade geometry. An initial attempt to protect shadow mode used the absolute MT5 tick timestamp as the freshness source.
**After:** Explicitly authorized intelligent submission uses a final broker-quote gate before MT5 validation/send. It rejects zero normalized volume, an inactive/unverified quote stream, excessive current spread, stop exposure exceeding the approved currency-risk budget, and a live price that invalidates stop/target geometry. The immediate order payload uses that validated quote. Shadow mode requires at least one newly arriving quote change and fails closed if the feed ceases activity for more than five seconds, without assuming that XM's displayed broker clock is aligned with the workstation UTC clock.

**Performance impact:** One final quote read and account-currency stop calculation per candidate reaching execution, plus a constant-time quote-signature comparison per observed quote. This is minor and justified for a demo swing/intraday engine.
**Trading impact:** Strategy setup decisions are unchanged. An approved setup can now be rejected before submission if broker conditions have become unsafe. Shadow observations process only demonstrably changing market quotes.
**Reliability impact:** Prevents executing on an unverified or inactive feed and prevents false shadow-test conclusions from static terminal data. A residual micro-window remains between local validation and broker fill; later execution reconciliation and deviation analysis are still necessary.

**Read-only validation:** On 2026-05-27, an initial wall-clock freshness check rejected `GOLD.i#` because its MT5 timestamp appeared approximately 10,792 seconds ahead of workstation UTC and sent no order. A subsequent read-only motion probe observed five price/tick changes in six seconds on the same symbol, proving the stream was active despite the broker-time offset. After correction, an eight-second intelligent shadow run processed 20 confirmed live quote updates, found no qualified signal and sent no order; the protected pipeline dry run reached MT5 validation with `$5.00` account-currency risk at `0.01` lot and sent no order. The corrected gate therefore measures recently observed tick activity rather than trusting absolute broker-clock alignment; any scheduled strategy shadow test must still demonstrate changing quote flow before a candidate is actionable.
