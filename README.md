...
`v3_candidate_safety` remains the 3R shadow/backtest safety profile for comparison. `v4_candidate_safety_2r` is a separate 2R candidate with the same safety filters, London Open exclusion, early-entry trap filtering, displacement verification, and profile-aware post-cost RR gates. Neither profile is live approval until its deployment-gated backtest is positive.
